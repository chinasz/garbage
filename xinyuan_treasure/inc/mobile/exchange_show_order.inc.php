<?php
	...
	//all order
	$sql = "select * from ".tablename($this->table['order'])." o inner join ".tablename($this->table['goods'])." g on o.order_goods = g.goods_id inner join ".tablename($this->table['goods_class'])." c on c.class_id = g.goods_class where o.order_user = :user";
	...
	//推荐
	$recommend_list = $query->from($this->table['goods'])->where(array('public_id'=>$public_id))->limit(1,2)->getall();
